...
struct AQIResponse: Codable {
    let latitude: Double
    let longitude: Double
    let current: CurrentAQIData?
}

struct CurrentAQIData: Codable {
    let time: String
    let interval: Int
    let us_aqi: Int?
    let pm2_5: Double?
...
class SolarAPIService {
    ...
    func fetchAirQualityData(latitude: Double, longitude: Double) async throws -> AQIResponse {
        let aqBaseURL = "https://air-quality-api.open-meteo.com/v1/air-quality"
        var components = URLComponents(string: aqBaseURL)!
        components.queryItems = [
            URLQueryItem(name: "latitude", value: "\(latitude)"),
            URLQueryItem(name: "longitude", value: "\(longitude)"),
            URLQueryItem(name: "current", value: "us_aqi,pm2_5,european_aqi,ozone"),
            URLQueryItem(name: "timezone", value: "auto"),
            URLQueryItem(name: "domains", value: "auto")
        ]
        
        guard let url = components.url else {
            print("❌ SolarAPIService (AQI): Bad URL: \(components.string ?? "N/A")")
            throw APIError.badURL
        }
        
        print("💨 SolarAPIService (AQI): Fetching AQI data from URL: \(url)")
        let (data, response) = try await URLSession.shared.data(from: url)
        
        guard let httpResponse = response as? HTTPURLResponse else {
            throw APIError.networkError(description: "AQI: Invalid server response.")
        }
        guard httpResponse.statusCode == 200 else {
            let responseBody = String(data: data, encoding: .utf8) ?? "No AQI response body"
            print("❌ SolarAPIService (AQI): HTTP Error: \(httpResponse.statusCode). Body: \(responseBody)")
            throw APIError.badServerResponse(statusCode: httpResponse.statusCode, responseBody: responseBody)
        }
        
        do {
            let decoder = JSONDecoder()
            let decodedResponse = try decoder.decode(AQIResponse.self, from: data)
            return decodedResponse
        } catch {
            print("❌ SolarAPIService (AQI): JSON Decoding Error: \(error.localizedDescription).")
            throw APIError.decodingError(description: "AQI: \(error.localizedDescription)", data: data)
        }
    }
}
